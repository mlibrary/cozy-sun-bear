...
export function popupTables(reader, contents) {
  var tables = contents.document.querySelectorAll('table,[data-resource-type]');
  var h = reader._rendition.manager.layout.height;
  var clipped_tables = [];
  for(var i = 0; i < tables.length; i++) {
    var table = tables[i];
    if ( table.offsetHeight >= ( h * 0.75 ) || table.dataset.resourceType ) {
  ...
  clipped_tables.forEach((table) => {
    ...
    if ( table.dataset.resourceType ) {
      // something different!

      var button = document.createElement('button');
      button.innerText = `Open ${table.dataset.resourceType.replace(/-/g, ' ')}`;

      var parent = table.querySelector('[data-resource-trigger]');
      if ( parent ) {
        parent.innerHTML = '';
      } else {
        parent = table;
      ...
      if ( table.localName == 'figure' ) {
        // DO MORE INTERESTING THINGS
      } else {
      }

      var iframe_href = table.dataset.href;
      var iframe_title = table.dataset.title;
